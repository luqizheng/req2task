...
import type { LLMMessage, LLMOptions, LLMResponse, StreamChunk } from './types.js';
...
export class OllamaProvider extends BaseProvider {
  ...
  async generate(messages: LLMMessage[], options?: LLMOptions): Promise<LLMResponse> {
    ...
    try {
      ...
      const response = await ollama.chat({ model, messages, stream: false, options: this.buildOptions(options) });
  ...
  async generateStream(messages: LLMMessage[], options?: LLMOptions): Promise<AsyncGenerator<StreamChunk>> {
    ...
    try {
      const response = await ollama.chat({ model, messages, stream: true, options: this.buildOptions(options) });

      for await (const part of response) {
        if (part.message?.content) chunks.push({ content: part.message.content, done: false });
        if (part.done) {
          usage = { promptTokens: part.prompt_eval_count || 0, completionTokens: part.eval_count || 0, totalTokens: (part.prompt_eval_count || 0) + (part.eval_count || 0) };
          finishReason = 'stop';
  ...
  private buildOptions(options?: LLMOptions): Record<string, unknown> {
    ...
    else if (this.config.maxTokens) opts.num_predict = this.config.maxTokens;

    if (options?.topP !== undefined) opts.top_p = options.topP;
    else if (this.config.topP !== undefined) opts.top_p = this.config.topP;
