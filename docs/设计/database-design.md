...
```sql
-- 典型查询模式
CREATE INDEX idx_xxx_deleted ON xxx(deleted_at) WHERE deleted_at IS NULL;
```
...
```sql
CREATE OR REPLACE FUNCTION update_module_story_points()
RETURNS TRIGGER AS $$
BEGIN
  UPDATE feature_modules
  SET estimated_story_points = (
    SELECT COALESCE(SUM(r.story_points), 0)
    FROM requirements r
    WHERE r.module_id = (
      SELECT module_id FROM requirements WHERE id = COALESCE(NEW.requirement_id, OLD.requirement_id)
    )
...
CREATE TRIGGER update_requirement_story_points
  AFTER INSERT OR UPDATE OR DELETE ON requirements
  FOR EACH ROW EXECUTE FUNCTION update_module_story_points();
```
...
RETURNS TRIGGER AS $$
DECLARE
  req_prefix VARCHAR(10);
  seq_num INTEGER;
BEGIN
  SELECT SUBSTRING(NEW.requirement_id::text, 1, 4) INTO req_prefix;

  SELECT COALESCE(MAX(
    CAST(SUBSTRING(task_number FROM 'REQ-\d{4}-TASK-(\d{3})') AS INTEGER)
  ), 0) + 1
  INTO seq_num
  FROM tasks
  WHERE requirement_id = NEW.requirement_id
  AND task_number LIKE 'REQ-' || req_prefix || '-TASK-%';

  NEW.task_number := 'REQ-' || req_prefix || '-TASK-' || LPAD(seq_num::text, 3, '0');
...
SELECT
  ...
  COUNT(DISTINCT CASE WHEN r.status = 'approved' THEN r.id END) AS approved_requirement_count,
  ...
  COALESCE(SUM(CASE WHEN t.is_wasted = TRUE THEN t.actual_hours ELSE 0 END), 0) AS wasted_hours
FROM projects p
LEFT JOIN feature_modules fm ON fm.project_id = p.id AND fm.deleted_at IS NULL
LEFT JOIN requirements r ON r.module_id = fm.id AND r.deleted_at IS NULL
...
```sql
CREATE VIEW v_requirement_details AS
SELECT
  r.id,
  r.title,
  r.description,
  ...
  COUNT(DISTINCT t.id) AS task_count
FROM requirements r
JOIN feature_modules fm ON fm.id = r.module_id
JOIN projects p ON p.id = fm.project_id
JOIN users u ON u.id = r.created_by_id
LEFT JOIN user_stories us ON us.requirement_id = r.id AND us.deleted_at IS NULL
LEFT JOIN acceptance_criteria ac ON ac.user_story_id = us.id AND ac.deleted_at IS NULL
LEFT JOIN tasks t ON t.requirement_id = r.id AND t.deleted_at IS NULL
WHERE r.deleted_at IS NULL
GROUP BY r.id, r.title, r.description, r.priority, r.status, r.story_points, r.source, fm.name, p.name, u.name;
```
...
|------|-----------------|------|
| UUID | UUID | 主键、外键 |
| 字符串 | VARCHAR/TEXT | 可变长度字符串 |
| 枚举 | VARCHAR + CHECK | 使用约束模拟枚举 |
| 日期时间 | TIMESTAMP | 带时区时间戳 |
| 日期 | DATE | 仅日期 |
| 数字 | DECIMAL/INTEGER | 精确数字 |
