...
function checkNodejs(): CheckResult {
  ...
  if (result.success && result.version) {
    ...
    return {
      ...
      message: passed
        ? version
        : `Version ${version} is too old, need >=${minMajor}.x`,
    };
  }
  return {
    name: "Node.js",
...
function checkPnpm(): CheckResult {
  ...
  if (result.success && result.version) {
    ...
    return {
      ...
      message: passed
        ? version
        : `Version ${version} is too old, need >=${minMajor}.0`,
    };
  }
  return {
    name: "pnpm",
...
async function main() {
  ...
  if (allPassed) {
    console.log(
      "\x1b[32m✅ All checks passed! Ready for development.\x1b[0m\n",
    );
    process.exit(0);
  } else if (criticalPassed) {
    console.log(
      "\x1b[33m⚠️  Core tools OK, but some optional tools are missing.\x1b[0m\n",
    );
    console.log(
    ...
    console.log(
      "\x1b[31m❌ Some required tools are missing. Please install them before continuing.\x1b[0m\n",
    );
    process.exit(1);
  }
}
